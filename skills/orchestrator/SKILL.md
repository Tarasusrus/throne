...
An executor that hit the vendor's usage limit is neither stopped nor gone: its session is alive in
tmux and waits for the reset, so `watch` shows it as `пауза до HH:MM (лимит вендора)` instead of
`работает`, and `list` marks the same row with `⏸`. Throne resumes it on its own — the vendor's
